...
- (void)loadLibaries
{
    ...
    // Sound Library
    filePath = [NSString stringWithFormat:@"%@/soundLibrary.lmlib", libraryFolder];
    if([[NSFileManager defaultManager] fileExistsAtPath:filePath isDirectory:&isDirectory])
    {
        soundLibrary = [[NSMutableDictionary alloc] initWithContentsOfFile:filePath];
    }
    else
    {
        soundLibrary = [[NSMutableDictionary alloc] init];
        NSMutableArray *soundFilePaths = [[NSMutableArray alloc] init];
        [soundLibrary setObject:soundFilePaths forKey:@"soundFilePaths"];
        [self setVersionNumberForSoundLibraryTo:LIBRARY_VERSION_NUMBER];
        // Create the folder
        NSError *error = nil;
        if(![[NSFileManager defaultManager] createDirectoryAtPath:[filePath stringByDeletingPathExtension] withIntermediateDirectories:YES attributes:nil error:&error]) 
        {
...
- (void)saveSoundLibrary
{
    NSString *filePath = [NSString stringWithFormat:@"%@/soundLibrary.lmlib", libraryFolder];
    [soundLibrary writeToFile:filePath atomically:YES];
}
...
{
    return [sequence objectForKey:@"soundFilePaths"];
}

- (int)soundFilePathsCountForSequence:(NSMutableDictionary *)sequence
{
    return (int)[[self soundFilePathsForSequence:sequence] count];
}

- (NSString *)soundFilePathAtIndex:(int)index forSequence:(NSMutableDictionary *)sequence;
{
    return [[self soundFilePathsForSequence:sequence] objectAtIndex:index];
}
...
- (NSString *)createSequenceAndReturnFilePath
{
    ...
    NSMutableArray *soundFilePathsForSequence = [[NSMutableArray alloc] init];
    ...
    [newSequence setObject:soundFilePathsForSequence forKey:@"soundFilePaths"];
...
- (NSString *)createCopyOfSequenceAndReturnFilePath:(NSMutableDictionary *)sequence
{
    ...
    [newSequence setObject:[self soundFilePathsForSequence:sequence] forKey:@"soundFilePaths"];
...
{
    NSMutableArray *filePaths = [self soundFilePathsForSequence:sequence];
    [filePaths addObject:filePath];
    [sequence setObject:filePaths forKey:@"soundFilePaths"];
    [self saveDictionaryToItsFilePath:sequence];
...
- (NSString *)soundFilePathForCommandCluster:(NSMutableDictionary *)commandCluster
{
    return [commandCluster objectForKey:@"soundFilePath"];
}
...
- (void)setSoundFilePath:(NSString *)filePath forCommandCluster:(NSMutableDictionary *)commandCluster
{
    [commandCluster setObject:filePath forKey:@"soundFilePath"];
    [self saveDictionaryToItsFilePath:commandCluster];
}
...
{
    return [self versionNumberForDictionary:soundLibrary];
}

- (NSMutableArray *)soundFilePaths
{
    return [soundLibrary objectForKey:@"soundFilePaths"];
}

- (NSString *)soundFilePathAtIndex:(int)index
{
    return [[self soundFilePaths] objectAtIndex:index];
}

- (int)soundFilePathsCount
{
    return (int)[[self soundFilePaths] count];
}

// Setter Methods
- (void)setVersionNumberForSoundLibraryTo:(float)newVersionNumber
{
    [self setVersionNumber:newVersionNumber forDictionary:soundLibrary];
    [self saveSoundLibrary];
}

- (void)addSoundFilePathToSoundLibrary:(NSString *)filePath
{
    NSMutableArray *filePaths = [self soundFilePaths];
    [filePaths addObject:filePath];
    [soundLibrary setObject:filePaths forKey:@"soundFilePaths"];
    [self saveSoundLibrary];
}
...
{
    return [sound objectForKey:@"filePathToAudioFile"];
}

- (float)startTimeForSound:(NSMutableDictionary *)sound
{
    return [[sound objectForKey:@"startTime"] floatValue];
}

- (float)endTimeForSound:(NSMutableDictionary *)sound
{
    return [[sound objectForKey:@"endTime"] floatValue];
}

// Setter Methods
- (void)setVersionNumber:(float)newVersionNumber forSound:(NSMutableDictionary *)sound
{
    [sound setObject:[NSNumber numberWithFloat:newVersionNumber] forKey:@"versionNumber"];
    [self saveDictionaryToItsFilePath:sound];
}

- (NSString *)createSoundAndReturnFilePath
{
    NSMutableDictionary *newSound = [[NSMutableDictionary alloc] init];
    NSMutableArray *beingUsedInSequenceFilePaths = [[NSMutableArray alloc] init];
    [newSound setObject:beingUsedInSequenceFilePaths forKey:@"beingUsedInSequenceFilePaths"];
    
    // New files get a file name chosen by availble numbers (detemined by @selector(nextAvailableNumberForFilePaths))
    NSString *filePath = [NSString stringWithFormat:@"soundLibrary/%@.lmsd", [self nextAvailableNumberForFilePaths:[self soundFilePaths]]];
    [self addSoundFilePathToSoundLibrary:filePath];
    [self setFilePath:filePath forDictionary:newSound];
    
    [newSound writeToFile:[NSString stringWithFormat:@"%@/%@", libraryFolder, filePath] atomically:YES];
    [self setVersionNumber:DATA_VERSION_NUMBER forSound:newSound];
    [self setDescription:@"New Sound" forSound:newSound];
    [self setFilePathToAudioFile:@"" forSound:newSound];
...
{
    NSString *newSoundFilePath = [self createSoundAndReturnFilePath];
    NSMutableDictionary *newSound = [self dictionaryFromFilePath:newSoundFilePath];
    [newSound setObject:[self dictionaryBeingUsedInSequenceFilePaths:sound] forKey:@"beingUsedInSequenceFilePaths"];
    [self setDescription:[NSString stringWithFormat:@"%@ Copy", [self descriptionForSound:sound]] forSound:newSound];
    [self setFilePathToAudioFile:[self filePathToAudioFileForSound:sound] forSound:newSound];
    [self setStartTime:[self startTimeForSound:sound] forSound:newSound];
    [self setEndTime:[self endTimeForSound:sound] forSound:newSound];
    
    return newSoundFilePath;
}

- (void)setDescription:(NSString *)description forSound:(NSMutableDictionary *)sound
{
    [sound setObject:description forKey:@"description"];
    [self saveDictionaryToItsFilePath:sound];
}

- (void)removeSoundFromLibrary:(NSMutableDictionary *)sound
{
    // Remove the sound from any sequences
    NSMutableArray *sequenceFilePaths = [self soundBeingUsedInSequenceFilePaths:sound];
    for(int i = 0; i < [sequenceFilePaths count]; i ++)
    {
        [self removeSoundFilePath:[self filePathForSound:sound] forSequence:[self sequenceFromFilePath:[sequenceFilePaths objectAtIndex:i]]];
    }
    
    NSMutableArray *filePaths = [self soundFilePaths];
    [filePaths removeObject:[self filePathForSound:sound]];
    NSString *filePath = [NSString stringWithFormat:@"%@/%@", [self libraryFolder], [self filePathToAudioFileForSound:sound]];
...
{
    [sound setObject:filePath forKey:@"filePathToAudioFile"];
    [self saveDictionaryToItsFilePath:sound];
}

- (void)setStartTime:(float)time forSound:(NSMutableDictionary *)sound
{
    [sound setObject:[NSNumber numberWithFloat:time] forKey:@"startTime"];
    [self saveDictionaryToItsFilePath:sound];
}

- (void)setEndTime:(float)time forSound:(NSMutableDictionary *)sound
{
    [sound setObject:[NSNumber numberWithFloat:time] forKey:@"endTime"];
    [self saveDictionaryToItsFilePath:sound];
}
