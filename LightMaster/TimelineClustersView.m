...
#import "TimelineClustersView.h"
#import "TimelineTracksView.h"

@interface TimelineClustersView()
...
- (void)drawSoundsAtTrackIndex:(int)index;
- (void)drawControlBoxCommandClustersAtTrackIndex:(int)index controlBoxIndex:(int)controlBoxIndex;
- (void)drawCommandsForCommandCluster:(NSMutableDictionary *)commandCluster atTrackIndex:(int)index forControlBoxOrGroup:(int)boxOrGroup;
- (void)drawGroupCommandClustersAtTrackIndex:(int)index groupIndex:(int)groupIndex;
...
@implementation TimelineClustersView
...
- (void)drawRect:(NSRect)dirtyRect
{
    ...
    tracksCount = 0;
    int soundsCount = 0, controlBoxCount = 0, groupCount = 0;
    int groupIndex = 0, controlBoxIndex = 0;
    // Draw the audio track
    if([data soundFilePathsCountForSequence:currentSequence] > 0)
    {
        [self drawTrackBackgroundAtIndex:tracksCount];
        [self drawSoundsAtTrackIndex:tracksCount];
        tracksCount ++;
        soundsCount ++;
    }
    ...
    {
        [self drawTrackBackgroundAtIndex:tracksCount];
        controlBoxIndex = (soundsCount > 0 ? tracksCount - soundsCount : tracksCount);
    ...
    {
        [self drawTrackBackgroundAtIndex:tracksCount];
        groupIndex = (soundsCount > 0 ? tracksCount - soundsCount : tracksCount);
        groupIndex = (controlBoxCount > 0 ? groupIndex - controlBoxCount : groupIndex);
        [self drawGroupCommandClustersAtTrackIndex:tracksCount groupIndex:groupIndex];
        tracksCount ++;
        groupCount ++;
    }
...
- (void)drawSoundsAtTrackIndex:(int)index
{
    for(int i = 0; i < [data soundFilePathsCountForSequence:currentSequence]; i ++)
    {
        NSMutableDictionary *currentSound = [data soundFromFilePath:[data soundFilePathAtIndex:i forSequence:currentSequence]];
        NSRect soundRect = NSMakeRect([data timeToX:[data startTimeForSound:currentSound]], self.frame.size.height - (index + 1) * TRACK_HEIGHT - TOP_BAR_HEIGHT + 1, [data widthForTimeInterval:[data endTimeForSound:currentSound] - [data startTimeForSound:currentSound]], TRACK_HEIGHT - 2);
        
        // Sound Mouse Checking here
        if([[NSBezierPath bezierPathWithRect:soundRect] containsPoint:mousePoint] && mouseAction == MNMouseUp && mouseEvent != nil)
        {
            [self drawRect:soundRect withCornerRadius:CLUSTER_CORNER_RADIUS fillColor:[NSColor colorWithDeviceRed:1.0 green:1.0 blue:1.0 alpha:0.7] andStroke:YES];
            selectedSound = currentSound;
            [[NSNotificationCenter defaultCenter] postNotificationName:@"SelectSound" object:selectedSound];
            mouseEvent = nil;
        }
        else
        {
            [self drawRect:soundRect withCornerRadius:CLUSTER_CORNER_RADIUS fillColor:[NSColor colorWithDeviceRed:1.0 green:0.0 blue:0.0 alpha:0.7] andStroke:YES];
            selectedSound = nil;
        }
    }
}
...
- (void)drawControlBoxCommandClustersAtTrackIndex:(int)index controlBoxIndex:(int)controlBoxIndex
{
    for(int i = 0; i < [data commandClusterFilePathsCountForSequence:currentSequence]; i ++)
    {
        ...
        if([[data controlBoxFilePathForCommandCluster:currentCommandCluster] isEqualToString:[data controlBoxFilePathAtIndex:controlBoxIndex forSequence:currentSequence]])
        {
            ...
            [self drawCommandsForCommandCluster:currentCommandCluster atTrackIndex:index forControlBoxOrGroup:MNControlBox];
...
- (void)drawGroupCommandClustersAtTrackIndex:(int)index groupIndex:(int)groupIndex
{
    ...
    {
        NSMutableDictionary *currentCommandCluster = [data commandClusterFromFilePath:[data commandClusterFilePathAtIndex:i forSequence:currentSequence]];
        // Command Cluster is for this group
        if([[data groupFilePathForCommandCluster:currentCommandCluster] isEqualToString:[data groupFilePathAtIndex:groupIndex forSequence:currentSequence]])
        {
            NSRect commandClusterRect = NSMakeRect([data timeToX:[data startTimeForCommandCluster:currentCommandCluster]], self.frame.size.height - (index + 1) * TRACK_HEIGHT - TOP_BAR_HEIGHT + 1, [data widthForTimeInterval:[data endTimeForCommandCluster:currentCommandCluster] - [data startTimeForCommandCluster:currentCommandCluster]], TRACK_HEIGHT - 2);
            ...
            // Draw the commands and check for mouse clicks
            [self drawCommandsForCommandCluster:currentCommandCluster atTrackIndex:index forControlBoxOrGroup:MNGroup];
...
- (void)drawCommandsForCommandCluster:(NSMutableDictionary *)commandCluster atTrackIndex:(int)index forControlBoxOrGroup:(int)boxOrGroup
{
    ...
    else if(boxOrGroup == MNGroup)
    {
        numberOfChannelsForCommandCluster = [data itemsCountForGroup:[data groupFromFilePath:[data groupFilePathForCommandCluster:commandCluster]]];
    }
